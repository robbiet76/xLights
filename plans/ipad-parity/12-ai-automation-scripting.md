...
| REST/HTTP automation server | other | ✅ | ❌ | ipad-missing | P1 | hard | infeasible | `xLightsAutomations.cpp` (`ProcessHttpRequest`, `StartAutomationListener`); 75-command dispatch. The desktop-only, read-only xLightsDesigner `layout/models` API reports controller-assignment brightness/gamma calibration and configured model preview pixel size with explicit provenance for companion-app physical-light reasoning; this adds no desktop UI and needs no iPad UI counterpart. iPad already shares the underlying `Model`/`ControllerConnection`/controller configuration surfaces, but cannot host the listener. The API also reports `mediaContentFingerprint` from shared `AudioManager::Hash()`; iPad exposes that hash as `XLSequenceDocument.mediaFileHash()`. iOS app-sandbox blocks listening-socket servers. |
