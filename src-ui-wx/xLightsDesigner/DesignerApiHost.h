...
inline std::string FormatDesignerApiArgbColor(std::uint32_t color) {
    std::ostringstream stream;
    stream << "#" << std::uppercase << std::hex << std::setfill('0') << std::setw(8) << color;
    return stream.str();
}
...
public:
    ...
    [[nodiscard]] api::models::SongStructureSummary readSongStructure() const {
        api::models::SongStructureSummary summary;
        if (_frame == nullptr || _frame->CurrentSeqXmlFile == nullptr) {
            return summary;
        }

        summary.sequenceOpen = true;
        const auto& manager = _frame->GetSequenceElements().GetSongStructureManager();
        summary.hasRegions = manager.AnyViewHasRegions();
        summary.activeViewIndex = manager.GetActiveViewIndex();
        summary.activeViewName = manager.GetActiveViewName();

        std::ostringstream revisionToken;
        revisionToken << "active:" << summary.activeViewIndex << "|views:" << manager.GetViewCount();
        summary.views.reserve(manager.GetViewCount());
        for (size_t viewIndex = 0; viewIndex < manager.GetViewCount(); ++viewIndex) {
            api::models::SongStructureViewSummary view;
            view.viewIndex = static_cast<int>(viewIndex);
            view.name = manager.GetViewName(viewIndex);
            view.active = view.viewIndex == summary.activeViewIndex;

            const auto& regions = manager.GetViewRegions(viewIndex);
            view.regions.reserve(regions.size());
            revisionToken << "|view:" << view.viewIndex << ":" << view.name << ":" << regions.size();
            for (const auto& region : regions) {
                view.regions.push_back({
                    region.id,
                    region.startTimeMS,
                    region.endTimeMS,
                    region.name,
                    detail::FormatDesignerApiArgbColor(region.colorARGB)
                });
                revisionToken << "|region:"
                              << region.id << ":"
                              << region.startTimeMS << ":"
                              << region.endTimeMS << ":"
                              << region.name << ":"
            ...
            summary.views.push_back(std::move(view));
        }
        summary.revisionToken = revisionToken.str();
        return summary;
    }
